...
/** Matches Channel Partner table export / “13 columns” reference (data columns only). */
const EXPORT_CP_COLUMN_DEFS = [
  { id: "cpId", label: "CP ID" },
  { id: "date", label: "Date" },
  ...
  { id: "reraNumbers", label: "RERA Numbers" },
  { id: "lastActivityDate", label: "Last Activity Date" },
  { id: "lastActivityRemark", label: "Last Activity Remark" },
] as const;

const DEFAULT_SELECTED_IDS = EXPORT_CP_COLUMN_DEFS.map((c) => c.id);

function buildInitialSelection() {
  return new Set<string>(DEFAULT_SELECTED_IDS);
}

export function ExportChannelPartnerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [mainEl, setMainEl] = useState<HTMLElement | null>(null);
  const [fileName, setFileName] = useState("Channel Partner");
  ...
  const selectAll = useCallback(() => {
    setSelectedColumns(new Set(EXPORT_CP_COLUMN_DEFS.map((c) => c.id)));
  }, []);
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="export-cp-title"
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-5 py-4">
          <h2 id="export-cp-title" className="text-lg font-semibold text-[#1e293b]">
            Export Channel Partner Data
          </h2>
          <button
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <div>
            <label className="mb-1.5 block text-[13px] font-bold text-[#1e293b]" htmlFor="export-cp-file-name">
              File Name
            </label>
            <input
              suppressHydrationWarning
              id="export-cp-file-name"
              type="text"
              value={fileName}
            ...
            <div className="overflow-hidden rounded-md border border-gray-200">
              ...
              {columnsExpanded ? (
                <div className="max-h-[min(280px,40vh)] overflow-y-auto">
                  {EXPORT_CP_COLUMN_DEFS.map((col, index) => {
